# 1. Написать функцию, которая в качестве аргументов принимает строку json, список полей, которые необходимо 
# обработать, список имён, которые нужно найти и функцию-обработчика имени, который срабатывает, 
...
def parse_json(json_str: str, keyword_callback, required_fields=None, keywords=None):
    if not json_str or required_fields is None or keywords is None:
        raise ValueError("Arguments: json_str, required_fields, keywords must take some value, not None or empty value")
...
def mean(k):
    ...
    def mean_decorator(func):
        def wrapper(*args, **kwargs):
            ...
            print("mean duration= {:.2f}\n".format(sum(func_duration)/len(func_duration)))
            return res
        return wrapper
